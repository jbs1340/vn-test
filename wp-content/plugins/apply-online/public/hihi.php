...
class SinglePostTemplate{
    ...
    public function application_form($post_id = 0){
        ...
        <form class="aol_app_form aol_app_form_<?php echo $post_id; echo " "?> " name="aol_app_form" id="aol_app_form" enctype="multipart/form-data"  data-toggle="validator">
            <?php
            ...
            $aol_form_button = apply_filters('aol_form_button', array('id' => 'aol_app_submit_button', 'value' => __('Submit', 'ApplyOnline'), 'class' => 'btn btn-org btn-submit button submit fusion-button button-large aol-form-button'));
            $attributes = NULL;
    ...
    public function aol_the_content($content){
        ...
        if(in_array($this_template, $aol_types) OR has_shortcode( $content, 'aol_form' )):
            ...
            if($_GET['view'] == 'ap') {
                $aol_content = '<div class="aol-single aol-wrapper">' . $title_features . $features . $title_form . $form . '</div>';
                ...
                $aol_content = '<div class="aol-single aol-wrapper">' . $content . $title_features . $features . '
                    ...
                    <a href="'.get_the_permalink($post).'&view=ap"><button class="jd-button-apply button read-more btn btn-org float-right">Apply</button> </a>
                    <a href="javascript:history.go(-1)"><button class="jd-button-back button read-more btn btn-outline-dark float-right ">Back</button> </a>
...
/*<a href="'.get_the_permalink($post).'&view=ap" ><button class="btn-lg button read-more btn btn-outline-dark float-right ">'.__( 'Apply', 'ApplyOnline' ).'</button></a>';*/
    ...
    function aol( $atts ) {
        ...
        if(!(empty($filters) OR $a['filter'] == 'no' )){
            ...
            foreach ($filters as $key => $filter){
                ...
                echo '<div class="col-md-3"><select name="'.$key.'" class="aol-filter-select form-control" onchange="jQuery(\'#list-job-form\').submit();" style="width:275px">' ;
                echo '<option value="">'. sprintf(_x('Select your job', 'Filter Dropdown', 'ApplyOnline'), $filxter['plural']).'</option>';
                $args = array(
            ...
            echo '<div class="col-md-offset-2 col-md-4"><input type=text placeholder="Enter keywords" class="form-control" name="key" value="'.htmlspecialchars($_GET['key']).'" style="height:10.2mm"></div>';
            echo '<div class=" col-md-3"><button class="fusion-button button btn btn-org btn-block aol-filter-button">'.__('Search', 'ApplyOnline').'</button></div>';
        ...
        if(!empty($posts)):
            // if(isset($_POST[$key]) OR $term->term_id == (int)$_POST[$key]):
                ...
                if($a['display'] == 'list'): echo '<li><a href="'.get_the_permalink($post).'">'.$post->post_title.'</a></li>';
                    ...
                    foreach($order as $index):
                        switch ($index):
                            ...
                            case 'body_close':
                                if($a['excerpt'] != 'no') echo '<p class="" id="style-1" style="text-align: justify"></p>';
                                ...
                                echo '<a href="'.get_the_permalink($post).'&view=ok" ><button class="btn-lg button read-more btn btn-outline-dark float-right  " style="margin-right: 10px;">'.__( 'View', 'ApplyOnline' ).'</button></a>';
