...
function aol_form_generator($fields, $fieldset = 0, $prepend = NULL, $post_id = 0){
    ...
    foreach($fields as $field):
        ...
        switch ($field['type']){
            ...
            break;
            case 'file':
                $form_output.=  $wrapper_start.'<input type="'.esc_attr($field['type']).'" name="'.$prepend.$field_key.'[]" class="form-control" id="'.$field_key.'" value="'. sanitize_text_field($field['val']).'" '.$req_class.' multiple>'.$wrapper_end;
                break;
            case 'number':
        ...
        if ( substr ( $key, 0, 9 ) == '_aol_app_' ) {
            ...
            elseif(is_array($val)){
                ...
                if(isset($val['file']) AND isset($val['type']))
                    if($val['type']==='image/jpeg'||$val['type']==='application/pdf'||$val['type']==='image/png') {
                        $cv_file[] = $val;
                        $val=null;
                    }
                elseif(isset($val['url']) AND isset($val['type']))
                    if($val['type']==='application/pdf'||$val['type']==='image/jpeg'||$val['type']==='image/png'){
                    $cv_file[]=$val;
                    $val=null;
                }
