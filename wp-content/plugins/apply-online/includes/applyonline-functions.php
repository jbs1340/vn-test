...
{
    $filters = array('pending' => __('Waiting for interview', 'ApplyOnline'), 'rejected' => __('Rejected', 'ApplyOnline'), 'interest'
    => __('Possible interest', 'ApplyOnline'), 'accepted' => __('Accepted', 'ApplyOnline'), 'priority' => __('Priority', 'ApplyOnline'), 'nextstep' => __('Next Interview', 'ApplyOnline'));
...
function aol_form_generator($fields, $fieldset = 0, $prepend = NULL, $post_id = 0)
{
    ...
    foreach ($fields as $field):
        ...
        if ($i == $break) {
            $wrapper_start = '</div> <div class="col-md-6"> <div class="form-group"><label for="' . $field_key . '">' . $required . sanitize_text_field($field['label']) . '</label>';
        } else {
        ...
        switch ($field['type']) {
            ...
            case 'text_area':
                $form_output .= $wrapper_start . '<textarea name="' . $prepend . $field_key . '" class="form-control" id="' . $field_key . '" ' . $req_class . ' aria-describedby="help' . $field_key . '" rows="6" >' . sanitize_textarea_field($field['val']) . '</textarea>' . $wrapper_end;
                break;

            case 'date':
                $form_output .= $wrapper_start . '<input type="text" name="' . $prepend . $field_key . '" class="form-control datepicker" id="' . $field_key . '" value="' . sanitize_text_field($field['val']) . '"  placeholder="' . __('e.g.', 'ApplyOnline') . ' ' . current_time(get_option('date_format')) . '" ' . $req_class . '  aria-describedby="help' . $field_key . '" readonly style="background-color: white;">' . $wrapper_end;
                break;
            ...
            case 'email':
                ...
                break;
            case 'file':
                $form_output .= $wrapper_start . '<input type="' . esc_attr($field['type']) . '" name="' . $prepend . $field_key . '[]" class="form-control" id="' . $field_key . '" value="' . sanitize_text_field($field['val']) . '" ' . $req_class . ' multiple style="display: none;">
                <div>
                <button type="button" class="btn btn-choose-file btn-org " onclick="document.getElementById(\'' . $field_key . '\').click()">Choose file</button>
                </div>
                <div id="progress " class="mt-2 progress">
                    <div class="progress-bar progress-bar-success"></div>
                 </div>   
                <div id="files" class="files mt-2">
</div>
                <input type="hidden" name="_aol_app_cv_validate_cv" id="cv_validate"/>
               ' . $wrapper_end;
                break;
            case 'number':
                $form_output .= $wrapper_start . '<input type="text" name="' . $prepend . $field_key . '" class="form-control" id="phone-number" value="' . sanitize_text_field($field['val']) . '" ' . $req_class . '>' . $wrapper_end;
                break;
            default:
                $form_output .= $wrapper_start . '<input type="' . esc_attr($field['type']) . '" name="' . $prepend . $field_key . '" class="form-control" id="' . $field_key . '" value="' . sanitize_text_field($field['val']) . '" ' . $req_class . '>' . $wrapper_end;
                break;
...
function aol_application_data($post, $parent = NULL)
{
        ...
        if (substr($key, 0, 9) == '_aol_app_') {
            ...
            elseif (is_array($val)) {
                foreach ($val as $v) {
                    ...
                    if (isset($v['file']) AND isset($v['type']))
                        if ($v['type'] === 'image/jpeg' || $v['type'] === 'application/pdf' || $v['type'] === 'image/png') {
                            $cv_file[] = $v;
                            $val = null;
                        } elseif (isset($v['url']) AND isset($v['type']))
                            if ($v['type'] === 'application/pdf' || $v['type'] === 'image/jpeg' || $v['type'] === 'image/png') {
                                $cv_file[] = $v;
                                $val = null;
    ...
    if ($cv_file != null) {
        $data[] = array('label' => '', 'value' => $cv_file);
    }
    return $data;
}

function aol_application_data_for_export($post, $parent = NULL)
{
    $keys = get_post_custom_keys($post->ID);
    $parent = get_post_meta($post->post_parent);
    $data = array();
    foreach ($keys as $key):
        if (substr($key, 0, 9) == '_aol_app_') {
            ...
            $val = get_post_meta($post->ID, $key, true);
            //Support to previuos versions where only URL was saved in the post meta.
            if (!filter_var($val, FILTER_VALIDATE_URL) === false) $val = esc_url($val);
