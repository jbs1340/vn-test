...
class Applyonline_Admin{
	...
	public function __construct( $plugin_name, $version ) {
        ...
        add_filter( 'bulk_actions-edit-aol_application', array($this,'register_export_actions') );

        add_filter( 'handle_bulk_actions-edit-aol_application', array($this,'export_action_handler'), 10, 3 );
    ...
    function export_action_handler( $redirect_to, $doaction, $post_ids ) {
	    if($doaction === 'export-applications')
        {
            $args = array(
                    'post__in'=>$post_ids,
	    ...
	    elseif ($doaction === 'export-all-applications') {
            $args = array(
                    'posts_per_page' => -1,
                'post_type' => 'aol_application'
            );
        }
        $posts = get_posts_for_export($args);
        if(!empty($posts)) {

//	        error_reporting(0);

            require_once ( __DIR__."/../includes/PHPExcel-1.8/Classes/PHPExcel.php");
            ...
            {
                $val_comment[$comment->comment_post_ID][]=$comment->comment_author.' : '. $comment->comment_content;
            }
            foreach($posts as $post) {
                //var_dump($post);die;
                $activeSheet->setCellValue("A".$row, $post->ID);
                $activeSheet->setCellValue("B".$row, $post->post_title);
                $terms = get_the_terms($post->ID,'aol_application_status');
                foreach ($terms as $term)
                {
                    $activeSheet->setCellValue("C".$row, $term->name);
                }
                $offer_questions = aol_application_data_for_export($post);

                $question_col = 'D';
                foreach ($offer_questions as $question) {
                    $activeSheet->setCellValue($question_col.$row, $question['value']);
                    $header_questions[$question_col] = $question['label'];
                    $question_col++;
                }
                $comment_col= $question_col;
                $comment_col++;
                for ($i = 0;$i< $post->comment_count; $i++)
                {
                    $activeSheet->setCellValueExplicit($comment_col.'1', 'Comment', PHPExcel_Cell_DataType::TYPE_STRING);
                    $activeSheet->setCellValueExplicit($comment_col.$row, $val_comment[$post->ID][$i], PHPExcel_Cell_DataType::TYPE_STRING);
                    $comment_col++;
                }
                $row ++;
            }
            ...
            $activeSheet->setCellValue('B1', 'Job title');
            $activeSheet->setCellValue('C1', 'Status');

            foreach ($header_questions as $col=>$value) {
                //$activeSheet->setCellValue($col.'1', $value);
                $activeSheet->setCellValueExplicit($col.'1', $value, PHPExcel_Cell_DataType::TYPE_STRING);
            }

            // Redirect output to a client’s web browser (Excel2007)
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment;filename="export-csv-'.date("y-m-d_h.i.s").'.xlsx"');
    ...
    function register_export_actions($bulk_actions) {
        $bulk_actions['export-applications'] = __( 'Export Application', 'export-application');
        $bulk_actions['export-all-applications'] = __( 'Export All', 'export-all-application');
        return $bulk_actions;
    }
	...
	public function enqueue_styles() {
        ...
        wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/jquery.loading.min.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
        ...
        wp_enqueue_script('customize-admin-script', plugin_dir_url(__FILE__).'js/admin-script.js', array( 'jquery' ), $this->version, false);
        wp_enqueue_script('customize-pdf-script', plugin_dir_url(__FILE__).'js/pdfob.min.js', array( 'jquery' ), $this->version, false);
       // wp_enqueue_script('customize-pdf-script', plugin_dir_url(__FILE__).'public/js/pdfob.min.js', array( 'jquery' ), $this->version, false);
    ...
    public function save_ad_closing($post_id){
       ...
       public function comments_fix($actions, $comment){
        ...
        public function applicants_list_columns( $columns ){
            $columns = array (
                ...
                'applicant'=> __( 'Name Candidate', 'ApplyOnline' ),
                'phone' => __( 'Phone', 'ApplyOnline' ),
                'title'    => __( 'Job Title', 'ApplyOnline' ),
        ...
        */
        ...
        public function applicants_list_columns_value( $column, $post_id ){
            ...
            $phone = $this->array_find(__('Phone','ApplyOnline'), $keys);
            ...
            switch ( $column ) {
                ...
                case 'phone' :
                    if($phone === FALSE):
                        ...
                        $applicant = get_post_meta( $post_id, $keys[ $phone ], TRUE );
                        if(is_object($applicant)) $applicant = NULL;
                        elseif(is_array($applicant))    $applicant = implode(',', $applicant);
                ...
                case 'taxonomy' :
                    ...
                    if ( ! empty( $terms ) ) {
                        $out = array ();
                        foreach ( $terms as $term ) {
                            if($term->name ==='Pending'){
                                $name = "Waiting for interview";
                            }else if($term->name === 'nextstep')
                            {
                                $name = "Next interview";
                            }
                            elseif($term->name === 'interest'){
                                $name = "Possible interest";
                            }
                            else{
                                $name = $term->name;
                            }
                            $out[] = sprintf( '<a href="%s">%s</a>', esc_url( add_query_arg( array ( 'post_type' => 'aol_application', 'aol_application_status' => $term->slug ), 'edit.php' ) ), esc_html( sanitize_term_field( 'name', $name, $term->term_id, 'jobpost_category', 'display' ) )
                            );
                        }
                        echo join( ', ', $out );
                    ...
                    <div class="container wrap">
                        ...
                        <table class="table table-sm table-bordered table-responsive">
                            <tbody>
                                <?php
                                    ...
                                    foreach ( $rows as $row ):
                                        if(is_array($row['value'])){
                                            echo '<tr><td>'. $row['label'] .'</td><td><a href="' . $row['value']['url'] . '">Attachment</a></td></tr>';
                                        }
                                        else {
                                ...
                                $terms = get_the_terms($post->ID,'aol_application_status');
                                foreach ( $terms as $term ){
                                        echo '<tr><td>Status</td><td>' . $term->name . '</td></tr>';
                                    }
        ...
        public function aol_application_post_editor (){
            ...
            if ( !empty ( $post ) and $post->post_type =='aol_application' ):
                ...
                <div class="alert alert-warning" role="alert">YOU <strong>MUST CHANGE THE STATUS</strong></div>
                    <div class="error_aol"></div>
                    <h3><?php the_title(); ?></h3>
                    <h3>
                        <?php 
                        /*
                            ...
                            if(is_array($row['value']))
                            {
                                foreach ($row['value'] as $item) {
                                    if ($item['type'] == 'application/pdf') {
                                        echo '<tr><td colspan="2" class="pdf-viewer" data-url="' . $item['url'] . '"></td></tr>';
                                    } elseif ($item['type'] == 'image/jpeg' || $item['type'] == 'image/png') {
                                        echo '<tr><td colspan="2" id=""><a href="' . $item['url'] . '"><img src="' . $item['url'] . '" style="width: 50%"/></a> </td></tr>';
                                    }
                                }
                                echo "<tr><td colspan='2' id='list-pdf'> </td> </tr>";
                            }
                            else
                                echo '<tr><td>' . $row['label'] . '</td><td>' . $row['value'] . '</td></tr>';
         ...
         * However the aurhor of the article failed to understand that this function only provides a READ ONLY access to Attachments to internal users only e.g. Administrator.
         */
        function output_attachment(){
            if( current_user_can('read_application') AND isset($_REQUEST['aol_attachment']) ){
                ...
                $path = urldecode($_REQUEST['aol_attachment']);
 ...
 class Applyonline_MetaBoxes{
	 ...
	 */
        ...
        function enqueue_date_picker(){
            ...
            wp_enqueue_script('admin_js', get_template_directory_uri() . '/admin-script.js', false, '1.0.0' );
        }
            ...
            <div class="submitpost">
                ...
                <div id="major-publishing-actions">
                    ...
                    <span class="spinner"></span>
                        ...
                        <?php  wp_nonce_field('get-comments','getcomment');?>
...
class Applyonline_Settings extends Applyonline_MetaBoxes{
    ...
    public function registers_settings(){
        ...
        register_setting( 'aol_settings_group', 'aol_application_close_message' );
